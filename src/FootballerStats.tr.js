...
// Version: 1.0.0
// A MediaWiki gadget for editing footballer infoboxes and career statistics on Turkish Wikipedia.
...
( function () {
	...
	const OTHER_COLUMN_KEY_PREFIX = 'trwiki-football-stats-helper-other-column';
	const COLUMN_STATE_KEY_PREFIX = 'trwiki-football-stats-helper-column-state';
	const FIELD_KEYS = [
		'team',
		'teamLink',
		'disableTeamLink',
	...
	const COMPETITION_NOTE_LABELS = {
		cupApps: 'Ulusal kupa',
		leagueCupApps: 'Lig kupası',
		continentalApps: 'Kıtasal',
		otherApps: 'Diğer'
	};
	let updateDate = '';
	let updateDateInput;
	...
	function assertVisualEditorSession( session ) {
		if ( !session || getVisualEditorSurface() !== session.surface ||
			...
			throw new Error( 'Düzenleme oturumu değişti. FootballerStats aracını yeniden açın.' );
		}
	}
	...
	async function readVisualEditorSession() {
		...
		if ( !surface ) {
			throw new Error( 'Görsel düzenleyici henüz hazır değil.' );
		}
		...
		if ( !cleanValue( session.source ) ) {
			throw new Error( 'Açık belgenin içeriği okunamadı.' );
		}
		return session;
	}
	...
	async function openVisualEditorTool() {
		...
		try {
			...
			if ( !hasSupportedFootballerInfobox( session.source ) ) {
				throw new Error( 'Açık belgede Futbolcu bilgi kutusu bulunamadı.' );
			}
			ensureUi();
			visualEditorSession = session;
			await loadSavedRows( session.source );
			assertVisualEditorSession( session );
			backdrop.querySelector( '.tfsh-apply' ).textContent = 'Uygula';
			refreshUpdateDate();
			backdrop.classList.add( 'is-open' );
			window.requestAnimationFrame( updateOptionalCompetitionColumns );
		} catch ( error ) {
			mw.notify( error.message || 'Görsel düzenleyiciden veri alınamadı.', { type: 'error' } );
		} finally {
	...
	function hasSupportedFootballerInfobox( source ) {
		return /\{\{\s*Futbolcu bilgi kutusu\b/i.test( source || '' );
	}

	function isSupportedSkin() {
		// Bilinen ve gelecekteki MediaWiki görünümlerinde çalışmayı engelleme.
	...
	function formatNamedLeagueCell( row, key ) {
		...
		const yearNamedLeague = leagueName.match( /^(\d{4})\s+(.+)$/ );
		const linkTarget = yearNamedLeague ?
			leagueName : `${ cleanValue( row.season ) } ${ leagueName }`.trim();
	...
	function buildGroupRowLines( group, totalRows = group.rows, noteNames = new Map(), headerNotes = new Set() ) {
		...
		if ( totalRows.length > 1 ) {
			lines.push( '|-' );
			const totalCells = [ 'colspan="2"|Toplam' ];
			activeStatPairs().forEach( ( [ appsKey, goalsKey ] ) => {
	...
	function buildTableWikitext( rows ) {
		...
		if ( groups.length > 1 ) {
			lines.push( '|-' );
			const grandCells = [ 'colspan="3"|Kariyer toplam\u0131' ];
			activeStatPairs().forEach( ( [ appsKey, goalsKey ] ) => {
				if ( appsKey === 'localLeagueApps' ) {
					grandCells.push( '-' );
				}
				const allDashes = tableRows.every( ( row ) => (
					[ appsKey, goalsKey ].every( ( key ) => /^[-\u2013\u2014\u2212]?$/.test( cleanValue( row[ key ] ) ) )
				) );
				if ( allDashes ) {
					grandCells.push( 'colspan="2"|-' );
				} else {
					[ appsKey, goalsKey ].forEach( ( key ) => {
						const sum = sumColumnWithUnknown( tableRows, key );
						grandCells.push( sum.unknown ? '?' : String( sum.total ) );
					} );
				}
			} );
			if ( grandHasUnknown ) {
	...
	function joinCompetitionItems( items ) {
		...
		return items.slice( 0, -1 ).join( ', ' ) + ' ile ' + items[ items.length - 1 ];
	}
	...
	function parseCompetitionEntries( note ) {
		...
		if ( detailed.length > 1 && !remainder.replace( /[\s,.]|\b(?:ve|ile)\b|i\u00e7erir|kapsar/g, '' ) ) {
			return detailed;
		}
		const entries = [];
	...
	function addCompetitionEntry( initial = {} ) {
		...
		name.setAttribute( 'aria-label', 'Müsabaka' );
		name.value = initial.name || '';
		const nameCell = document.createElement( 'td' );
		nameCell.appendChild( name );
		element.appendChild( nameCell );
		const controls = {};
		[ [ 'apps', 'Maç' ], [ 'goals', 'Gol' ] ].forEach( ( [ key, label ] ) => {
		...
		for ( const entry of entries ) {
			for ( const key of [ 'apps', 'goals' ] ) {
				...
				if ( ( key === 'apps' || value ) && !/^\d+$/.test( value ) ) {
					entry[ key ].setCustomValidity( 'Her müsabaka için sıfır veya daha büyük bir tam sayı girin.' );
	...
	function competitionPair( row, appsKey, goalsKey, noteNames, headerNotes = new Set() ) {
		...
		if ( note && noteNames.has( note ) ) {
			return { ...pair, note: `{{adn|ad=${ noteNames.get( note ) }}}` };
		}
		if ( note && !noteNames.has( note ) ) {
			noteNames.set( note, 'not' + ( noteNames.size + 1 ) );
		}
		return { ...pair, note: note ? `{{adn|ad=${ noteNames.get( note ) }|1=${ competitionNoteSentence( note ) }}}` : '' };
	}

	function splitCompetitionNote( value, namedNotes = new Map() ) {
		const match = /\{\{\s*adn\s*\|/i.exec( value );
		if ( !match ) {
			return { value, note: '' };
		}
		let depth = 0;
		for ( let i = match.index; i < value.length - 1; i++ ) {
			...
			if ( pair === '{{' ) {
				...
				if ( depth === 0 ) {
					...
					const nameParameter = parameters.find( ( item ) => item.name === 'ad' );
					...
					const note = cleanValue( content ).replace( / ma\u00e7lar\u0131n\u0131 (?:i\u00e7erir|kapsar)\.$/, '' );
					if ( noteName && note ) {
	...
	function buildCareerSection( rows ) {
		const updated = updateDate ? `{{Güncellendi|${ updateDate }}}\n` : '';
		const clubHeading = nationalTeamCareerEnabled ? '=== Kulüp ===\n' : '';
		const sectionHeading = managerCareerEnabled ?
			'Futbolculuk istatistikleri' : 'Kariyer istatistikleri';
	...
	function isFootballerStatsTable( tableText ) {
		const headerText = tableText.split( /\r?\n/ )
			...
			.join( ' ' )
			.toLocaleLowerCase( 'tr-TR' );
		return /\btakım\b/.test( headerText ) &&
			/\bsezon\b/.test( headerText ) &&
			/colspan\s*=\s*"3"\s*\|\s*lig\b/.test( headerText ) &&
			/maç(?:lar)?/.test( headerText ) &&
	...
	function isInfoboxCareerParameter( name ) {
		return /^(kulüpyıl|kulüp|maç|gol)\d+$/i.test( cleanValue( name ) );
	}
	...
	function removeAutomaticTotalsAndEmptyCareerGroups( templateText ) {
		const parameters = topLevelTemplateParameters( templateText );
		...
		const familyPatterns = [
			{
				family: 'club',
				pattern: /^(kulüpyıl|kulüp|maç|gol)(\d+)$/i,
				yearNames: [ 'kulüpyıl' ],
				teamNames: [ 'kulüp' ]
			},
			{
				family: 'youth',
				pattern: /^(altyapıyıl|altyapıkulüp|altyapıkulübü)(\d+)$/i,
				yearNames: [ 'altyapıyıl' ],
				teamNames: [ 'altyapıkulüp', 'altyapıkulübü' ]
			},
			{
				family: 'national',
				pattern: /^(milliyıl|millitakım|milli?maç|milligol)(\d+)$/i,
				yearNames: [ 'milliyıl' ],
				teamNames: [ 'millitakım' ]
			},
			{
				family: 'manager',
				pattern: /^(çalıştığıyıl|çalıştığıkulüp)(\d+)$/i,
				yearNames: [ 'çalıştığıyıl' ],
				teamNames: [ 'çalıştığıkulüp' ]
			}
		];

		parameters.forEach( ( parameter ) => {
			const name = cleanValue( parameter.name ).toLocaleLowerCase( 'tr-TR' );
			if ( /^(toplammaç|toplamgol)$/i.test( name ) ) {
				removals.add( parameter );
				return;
			}
	...
	function ensureMissingYouthYearParameters( templateText ) {
		...
		parameters.forEach( ( parameter ) => {
			const name = cleanValue( parameter.name ).toLocaleLowerCase( 'tr-TR' );
			const yearMatch = name.match( /^altyapıyıl(\d+)$/i );
			if ( yearMatch ) {
		...
		const insertions = parameters
			.map( ( parameter ) => {
				const name = cleanValue( parameter.name ).toLocaleLowerCase( 'tr-TR' );
				const teamMatch = name.match( /^altyapıkul(?:üp|übü)(\d+)$/i );
				if ( !teamMatch || existingYears.has( teamMatch[ 1 ] ) ) {
					return null;
				}
				return {
					start: parameter.start,
					text: `| altyapıyıl${ teamMatch[ 1 ] } = \n`
				};
			} )
			.filter( Boolean )
	...
	function normalizeInfoboxParameterSpacing( templateText ) {
		const replacements = topLevelTemplateParameters( templateText )
			.filter( ( parameter ) => !/^madalyalar$/i.test( cleanValue( parameter.name ) ) )
			.map( ( parameter ) => {
	...
	function reorderNationalTeamParameters( templateText ) {
		const fieldOrder = [ 'milliyıl', 'millitakım', 'millimaç', 'milligol' ];
		const parameters = topLevelTemplateParameters( templateText );
		const groups = new Map();
		parameters.forEach( ( parameter ) => {
			const match = cleanValue( parameter.name ).toLocaleLowerCase( 'tr-TR' )
				.match( /^(milliyıl|millitakım|millimaç|milligol)(\d+)$/i );
			if ( !match ) {
				return;
			}
			const index = match[ 2 ];
			const group = groups.get( index ) || new Map();
			group.set( match[ 1 ].toLocaleLowerCase( 'tr-TR' ), parameter );
		...
		const nationalParameters = parameters.filter( ( parameter ) => /^(milliyıl|millitakım|millimaç|milligol)\d+$/i.test(
	...
	function extractAndRemoveManagerParameters( templateText ) {
		const parameters = topLevelTemplateParameters( templateText )
			.filter( ( parameter ) => /^çalıştığı/i.test( cleanValue( parameter.name ) ) );
	...
	function currentUpdateDate() {
		...
		return `${ today.getDate() } ${ UPDATE_MONTHS[ today.getMonth() ] } ${ today.getFullYear() }`;
	}

	function formatInfoboxUpdateDate( value ) {
		const text = stripHtmlComments( cleanValue( value ) )
		...
		let year, month, day;
		const iso = /\b(\d{4})-(\d{1,2})-(\d{1,2})(?!\d)/.exec( text );
		const numeric = /\b(\d{1,2})[./-](\d{1,2})[./-](\d{4})\b/.exec( text );
		const named = /\b(\d{1,2})\s+([\p{L}]+)\s+(\d{4})\b/u.exec( text );
		if ( iso ) {
			...
			day = Number( named[ 1 ] );
			year = Number( named[ 3 ] );
			month = UPDATE_MONTHS.findIndex( ( name ) => name.toLocaleLowerCase( 'tr' ) === named[ 2 ].toLocaleLowerCase( 'tr' ) ) + 1;
		}
		const date = new Date( Date.UTC( year, month - 1, day ) );
		return month && date.getUTCFullYear() === year && date.getUTCMonth() === month - 1 && date.getUTCDate() === day ?
			day + ' ' + UPDATE_MONTHS[ month - 1 ] + ' ' + year : text;
	}
	...
	async function resolveUpdateDateTemplate( value, loadId ) {
		...
		try {
			...
			mw.log.warn( 'Güncelleme tarihi şablonu çözümlenemedi:', error );
		}
	}
	...
	function initializeUpdateDate( source ) {
		...
		const parameter = bounds && topLevelTemplateParameters( bounds.lines.slice( bounds.startIndex, bounds.endIndex + 1 ).join( '\n' ) )
			.find( ( item ) => cleanValue( item.name ).toLocaleLowerCase( 'tr' ) === 'güncelleme' );
	...
	function syncUpdateDate( active ) {
		...
		if ( updateDateInput ) {
			...
			updateDateInput.placeholder = active ? currentUpdateDate() : 'Kariyeri sonlanmıştır';
	...
	function sourceHasOpenEndedClubYear( source ) {
		...
		return topLevelTemplateParameters( infoboxText ).some( ( parameter ) => (
			/^kulüpyıl\d+$/i.test( cleanValue( parameter.name ) ) &&
	...
	function hasManagerCareerData( source ) {
		...
		const infoboxText = bounds.lines.slice( bounds.startIndex, bounds.endIndex + 1 ).join( '\n' );
		return topLevelTemplateParameters( infoboxText ).some( ( parameter ) => (
			/^çalıştığıyıl\d+$/i.test( cleanValue( parameter.name ) ) &&
	...
	function updateInfoboxUpdateParameter( templateText ) {
		const parameters = topLevelTemplateParameters( templateText ).filter(
			( item ) => cleanValue( item.name ).toLocaleLowerCase( 'tr' ) === 'güncelleme'
		);
		let withoutUpdate = templateText;
		parameters.reverse().forEach( ( parameter ) => {
		...
		const closingIndex = withoutUpdate.lastIndexOf( '}}' );
		if ( closingIndex === -1 ) {
			return withoutUpdate;
		}
		const beforeClosing = withoutUpdate.slice( 0, closingIndex ).replace( /\s*$/, '' );
		return `${ beforeClosing }\n| güncelleme = ${ updateDate }\n${ withoutUpdate.slice( closingIndex ) }`;
	}
	...
	function hasNationalTeamCareerData( source ) {
		...
		if ( bounds ) {
			...
			const hasNationalTeamParameter = topLevelTemplateParameters( infoboxText ).some(
				( parameter ) => /^millitakım\d+$/i.test( cleanValue( parameter.name ) ) &&
		...
		return findWikitableRanges( source ).some( ( range ) => {
			...
			while ( ( match = headingRegex.exec( precedingSource ) ) !== null ) {
				nearestHeading = cleanValue( match[ 2 ] ).toLocaleLowerCase( 'tr-TR' );
			}
			return /mill[iî]\s+takım|uluslararası\s+istatistik/.test( nearestHeading );
		} );
	}

	function formatSeasonRange( start, end, separator ) {
		...
		return `${ start }${ separator }${ displayedEnd }`;
	}
	...
	function shortenInfoboxYearRange( value ) {
		const shortened = removeUnknownInfoboxStartYear( value ).replace(
			/(\b\d{4}\b)(\s*[-–—]\s*)(\d{4}\b)/g,
	...
	function parseRowsFromInfobox( source ) {
		...
		topLevelTemplateParameters( infoboxText ).forEach( ( parameter ) => {
			const match = parameter.name.match( /^(kulüpyıl|kulüp|maç|gol)(\d+)$/i );
			if ( !match ) {
				return;
			}
			...
			if ( field === 'kulüpyıl' ) {
				...
				const normalizedValue = removeUnknownInfoboxStartYear( seasonValue.value ).replace( /[–—−]/g, '-' );
	...
	function addInfoboxYearsToTableRows( rows, source ) {
		...
		periodRows.forEach( ( matches ) => {
			...
			if ( matches.length === 1 ) {
				matches[ 0 ].infoboxYear = original;
			}
		} );
		// Tabloda bulunmayan dönemleri ekle; kısmen eşleşen toplamları çoğaltma.
	...
	async function getInitialSourceForForm() {
		...
		// Bazı düzenleyiciler #wpTextbox1 öğesini içeriği yüklemeden önce oluşturur.
		// Boş textarea'yı kaynak kabul etmek yerine madde metnini API'den getir.
	...
	function matchingTeamLinkRows( sourceRow, previousLink ) {
		...
		const team = cleanValue( sourceInputs && sourceInputs.team.value ).toLocaleLowerCase( 'tr-TR' );
		...
		return Array.from( tbody.querySelectorAll( 'tr' ) ).filter( ( candidate ) => {
			...
			return inputs && ( candidate === sourceRow ||
				( team && cleanValue( inputs.team.value ).toLocaleLowerCase( 'tr-TR' ) === team &&
	...
	function createRow( initialData = {} ) {
		...
		data.infoboxYear = cleanValue( initialData.infoboxYear );
		...
		fieldDefs.forEach( ( [ key, type, className ] ) => {
			...
			if ( key === 'team' ) {
				...
				if ( key === 'teamLink' ) {
					const toggle = createLinkToggle( 'Bağlantı yok', data.disableTeamLink, () => {
					...
					const toggle = createLinkToggle( 'Bağlantı yok', data.disableSeasonLink, () => {
						input.disabled = toggle.checkbox.checked;
						refreshPreview();
					} );
					row.appendChild( toggle.label );
			...
			if ( COMPETITION_NOTE_LABELS[ key ] ) {
				...
				noteButton.title = COMPETITION_NOTE_LABELS[ key ] + ' müsabakalar notu';
			...
			if ( key === 'infoboxOnly' ) {
				const label = document.createElement( 'label' );
				label.className = 'tfsh-link-toggle';
				label.title = 'Bu satırı sezonluk tablodan çıkarır, bilgi kutusunda korur';
				label.appendChild( document.createTextNode( 'Tabloda yok' ) );
		...
		removeButton.title = 'Satırı sil';
		removeButton.setAttribute( 'aria-label', 'Satırı sil' );
	...
	function removeSeasonRow( tr ) {
		...
		if ( exit && previous && teamGroupKey( previous.tfshData.inputs ) === key &&
			...
			if ( !beforePrevious || teamGroupKey( beforePrevious.tfshData.inputs ) !== key ) {
				...
				input.value = !entry || entry === exit ? exit : entry + ( /^[-–—−]$/.test( exit ) ? '-' : '-' + exit );
			} else {
				input.value = exit;
			}
		...
		if ( sourceTr ) {
			...
			if ( peers[ peers.length - 1 ] === sourceTr ) {
				...
				if ( peers.length > 1 ) {
					...
					seed.infoboxYear = range ? ( range[ 2 ] || '-' ) : exitYear;
					if ( range ) {
	...
	function initializeColumnGuides() {
		const guides = [
			[ 'Takım', 'Bilgi kutusu ve tabloda görünen takım adıdır.\n\n* Bilgi kutusunda bir takım bilgisi girilmişse, onun görünen adı otomatik olarak yazılır.\n* Futbolcu takımda kiralık oynamışsa "kiralık" kutusunu işaretleyin.\n* Takımın bilgi kutusunda gözükmesini ancak istatistik tablosunda gözükmemesini istiyorsanız "tabloda yok" kutusunu işaretleyin.' ],
			[ 'Takım bağlantısı', 'Takımın Vikipedi maddesinin adıdır.\n\n* Bilgi kutusunda bir takım bağlantısı varsa, bu bağlantı otomatik olarak yazılır.\n* Mevcut bağlantı bir yönlendirme ise, yönlendirme bağlantısı otomatik olarak düzeltilir.' ],
			[ 'Sezon', 'Futbolcunun istatistik bilgisinin ekleneceği sezondur.\n\n* Bilgi kutusunda yıl bilgileri varsa, ona göre otomatik yazılır.\n* Herhangi bir takıma sezon eklemek için, o takım sezonlarının olduğu bölümdeki + düğmesine basın.' ],
			[ 'Bilgi kutusu yılı', 'Futbolcunun kulübe geliş ve kulüpten çıkış yıllarının bilgi kutusunda nasıl görüneceğini yazın.\n\n* Bir yıl oynamışsa yalnızca o yılı yazın.\n* İki yıl oynamışsa geliş yılı ile gidiş yılını arasına kısa çizgi (-) koyarak yazın.\n* Birden fazla yıl kalmışsa sezonlarının ilk satırına geliş, son satırına gidiş yılını yazın.\n* Aynı takımda oynamaya devam ediyorsa kısa çizgi (-) kullanın.' ],
			[ 'Sezon bağlantısı', 'Takımın sezon maddesinin bağlantısını girin.\n\n* Sezon maddesine bağlantı verilmemesi gerekiyorsa "bağlantı yok" seçeneğini seçin.' ],
			[ 'Lig', 'İstatistik tablosunda futbolcunun o sezon oynadığı ana ligdir.\n\n* Lig adını girdiğinizde, aynı takımın tüm sezonlarına aynı lig adı otomatik olarak girilir. Takım, farklı sezonlarda farklı liglerde yer almışsa bu bilgiler değiştirilmelidir.\n* Sezon kısmındaki değer algılanarak ligin o sezonunun maddesine bağlantı oluşturulur.\n* Ligin o sezon kullanılan adı yazılmalıdır.\n* Lig sezonu maddesi, sezon sütunundaki yıldan farklıysa, sezon değeri de lig adının önüne yazılmalıdır.\n* Futbolcunun, ligde çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım lige katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Lig düzenlenmedi ya da takım katılmadı: Boş' ],
			[ 'Yerel lig', 'İstatistik tablosunda futbolcunun o sezon oynadığı yerel ligdir.\n\n* Yalnızca futbolcunun aynı sezonda hem yerel hem ulusal lig maçlarına çıktığı durumda kullanılmalıdır.\n* Lig adını girdiğinizde, aynı takımın tüm sezonlarına aynı lig adı otomatik olarak girilir. Takım, farklı sezonlarda farklı liglerde yer almışsa bu bilgiler değiştirilmelidir.\n* Sezon kısmındaki değer algılanarak ligin o sezonunun maddesine bağlantı oluşturulur.\n* Ligin o sezon kullanılan adı yazılmalıdır.\n* Lig sezonu maddesi, sezon sütunundaki yıldan farklıysa, sezon değeri de lig adının önüne yazılmalıdır.\n* Futbolcunun, ligde çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım lige katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Lig düzenlenmedi ya da takım katılmadı: Boş' ],
			[ 'Ulusal kupa', 'İstatistik tablosunda futbolcunun o sezon oynadığı ulusal kupa müsabakasıdır.\n\n* Futbolcunun oynadığı sezonların hiçbirinde ulusal kupa müsabakası mevcut değilse, başlığa tıklayarak sütunu kapatın.\n* Futbolcunun, ulusal kupada çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım müsabakaya katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Müsabaka düzenlenmedi ya da takım katılmadı: Boş\n* "N" düğmesine tıklayıp çıkan penceredeki alana, hangi ulusal kupa müsabakasının kapsandığını yazın.' ],
			[ 'Lig kupası', 'İstatistik tablosunda futbolcunun o sezon oynadığı lig kupası müsabakasıdır.\n\n* Futbolcunun oynadığı sezonların hiçbirinde  lig kupası müsabakası mevcut değilse, başlığa tıklayarak sütunu kapatın.\n* Futbolcunun, lig kupasında çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım müsabakaya katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Müsabaka düzenlenmedi ya da takım katılmadı: Boş\n* "N" düğmesine tıklayıp çıkan penceredeki alana, hangi lig kupası müsabakasının kapsandığını yazın.' ],
			[ 'Kıtasal', 'İstatistik tablosunda futbolcunun o sezon oynadığı kıtasal müsabakalardır.\n\n* Futbolcunun oynadığı sezonların hiçbirinde kıtasal müsabaka mevcut değilse, başlığa tıklayarak sütunu kapatın.\n* Futbolcunun, kıtasal müsabakalarda çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım müsabakaya katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Müsabaka düzenlenmedi ya da takım katılmadı: Boş\n* "N" düğmesine tıklayıp çıkan penceredeki alana, hangi kıtasal müsabakaların kapsandığını yazın.' ],
			[ 'Diğer', 'İstatistik tablosunda futbolcunun o sezon oynadığı, diğer sütunlarda yer almayan müsabakalardır.\n\n* Futbolcunun oynadığı sezonların hiçbirinde bu sütuna eklenecek başka bir müsabaka mevcut değilse, başlığa tıklayarak sütunu kapatın.\n* Futbolcunun, bu müsabakalarda çıktığı maç ve bu maçlarda attığı gol bilgileri yazılmalıdır.\n* Maç ve gol bilgileri şu şekilde girilmelidir:\n  * Takım müsabakaya katıldı, futbolcu maça çıkmadı: 0\n  * İlgili maç ya da gol sayısı bilinmiyor: ?\n  * Müsabaka düzenlenmedi ya da takım katılmadı: Boş\n* "N" düğmesine tıklayıp çıkan penceredeki alana, hangi müsabakaların kapsandığını yazın.' ]
		];
		...
		guides.forEach( ( [ title, text ], index ) => {
			...
			button.textContent = '?';
			button.title = title + ' kullanım kılavuzu';
	...
	function editCompetitionNote( event, row, key, button ) {
		...
		const title = [ cleanValue( row.tfshData.inputs.season.value ), COMPETITION_NOTE_LABELS[ key ].toLocaleLowerCase( 'tr-TR' ), 'notu' ].filter( Boolean ).join( ' ' );
	...
	async function loadSavedRows( sourceOverride ) {
		...
		try {
			...
			if ( Object.keys( getSavedColumnStates() ).length ) {
				...
				continentalEnabled = parsedColumnStates.continental;
	...
	function isNationalStatisticsTable( table ) {
		const text = stripHtmlComments( table ).replace( /<ref\b[^>]*>[\s\S]*?<\/ref>|<ref\b[^>]*\/>/gi, '' )
			.toLocaleLowerCase( 'tr-TR' );
		const hasTeamLink = /\[\[[^\]\n|]*mill[iî] futbol takımı(?:#[^\]\n|]*)?(?:\|[^\]\n]*)?\]\]/.test( text );
		const headers = text.split( /\r?\n/ ).filter( ( line ) => /^\s*!/.test( line ) ).join( ' ' );
		return hasTeamLink && /(?:^|[^a-zçğıöşü])(?:maç(?:lar)?|gol(?:ler)?)(?=$|[^a-zçğıöşü])/.test( headers );
	}
	...
	function insertClubBeforeNationalStatistics(
		...
		if ( clubTable ) {
			...
			if ( /^==[^=\n]+==[ \t]*$/m.test( extracted.source.slice( clubEnd, nationalOffset ) ) ) {
				beforeClub = beforeClub.replace(
					/(?:^|\n)==[ \t]*(?:Kariyer|Futbolculuk) istatistikleri[ \t]*==\s*$/, '\n'
				);
			}
		...
		const statsHeading = headings.find( ( heading ) => (
			heading.level === 2 &&
			/^(?:Kariyer|Futbolculuk) istatistikleri$/i.test( heading.title )
		) );
		const nationalHeadings = headings.filter( ( heading ) => (
			heading !== statsHeading &&
			/istatistik|^mill[iî] takım$/i.test( heading.title.toLocaleLowerCase( 'tr-TR' ) )
		) );
		const nationalHeading = nationalHeadings[ 0 ];
		const title = hasManagerCareerData( source ) || /^== Futbolculuk istatistikleri ==/m.test( sectionText ) ||
			( statsHeading && statsHeading.title === 'Futbolculuk istatistikleri' ) ? 'Futbolculuk istatistikleri' : 'Kariyer istatistikleri';
		...
		const separator = statsHeading && before.trimEnd().endsWith( '== ' + title + ' ==' ) ?
			'\n' : '\n\n';
		return before.trimEnd() + separator + mainHeading + '=== Kulüp ===\n' + body +
			'\n\n=== Millî takım ===\n' + after.replace( /^\s*/, '' );
	}
	...
	function findExistingClubStatistics( source ) {
		...
		return ranges.find( ( range ) => {
			...
			const context = headings.map( ( item ) => item.title ).join( ' ' )
				.toLocaleLowerCase( 'tr-TR' );
			if ( /mill[iî]|uluslararası|teknik direktör|antrenör|performans|başarı|turnuva|derbi/.test( context ) ) {
				return false;
			}
			...
			const statIndicators = [ /maç(?:lar)?/, /gol(?:ler)?/, /toplam/ ];
			const contextIndicators = [ /kulüp|takım/, /sezon/, /lig/, /kupa/, /kıta|kıtasal/, /avrupa/ ];
			return statIndicators.some( ( indicator ) => indicator.test( headers ) ) &&
				contextIndicators.filter( ( indicator ) => indicator.test( headers ) )
					.length >= 2 &&
				!/mill[iî]|galibiyet|mağlubiyet|averaj|puan|antrenör/.test( headers );
		} );
	}
	...
	function reorderInfoboxDisplayParameters( templateText ) {
		let result = templateText;
		[ [ 'altyazı', [ 'resimboyutu', 'resim' ] ], [ 'numarası', [ 'bulunduğukulüp' ] ] ]
	...
	function normalizeClubStatisticsHeadings( source, table, sectionText ) {
		...
		const index = headings.findIndex( ( heading ) => (
			/istatistik/.test( heading.title.toLocaleLowerCase( 'tr-TR' ) )
		) );
		if ( index === -1 ) {
			return source;
		}
		...
		const title = hasManagerCareerData( source ) ||
			/Futbolculuk istatistikleri/.test( sectionText + oldHeadings[ 0 ].title ) ?
			'Futbolculuk istatistikleri' : 'Kariyer istatistikleri';
	...
	function insertOrReplaceCareerSection( source, sectionText ) {
		...
		return source.slice( 0, bodyStart ) + insertOrReplaceCareerSectionInBody(
			...
			/^(==[ \t]*(?:Kariyer|Futbolculuk) istatistikleri[ \t]*==)[ \t]*\r?\n(?:[ \t]*\r?\n)*(?====[ \t]*Kulüp[ \t]*===[ \t]*\r?$)/gm,
			'$1\n'
		);
	}
	...
	function insertOrReplaceCareerSectionInBody( source, sectionText ) {
		...
		if ( table ) {
			...
			if ( nationalTeamCareerEnabled ) {
				body = /^=== Kulüp ===/.test( body ) ? body : '=== Kulüp ===\n' + body;
			}
		...
		const closingHeadingPattern = /^(başarıları?|başarı ve ödülleri|ödülleri|notlar|kaynakça|kaynaklar|dış bağlantılar)$/;
		...
		// Bölümü taslak şablonları, sıralama ve kategorilerin
		// üstünde tut. Kariyer başlığı metnin sonuna işaret etse bile bu sınırı uygula.
		const trailingMetadata = /^(?:\s*\{\{[^\n{}]*(?:taslak|stub)[^\n{}]*\}\}\s*$|\{\{\s*(?:DEFAULTSORT|VarsayılanSıralama)\s*:|\[\[(?:Kategori|Category):)/gim.exec( cleanSource );
		if ( trailingMetadata && ( insertAt === -1 || trailingMetadata.index < insertAt ) ) {
			insertAt = trailingMetadata.index;
		}
		const trailingTemplate = /^(?:\s*\{\{[^\n{}]+\}\}\s*\r?\n)+(?=\s*(?:\[\[(?:Kategori|Category):|$))/im.exec( cleanSource );
		if ( trailingTemplate && ( insertAt === -1 || trailingTemplate.index < insertAt ) ) {
			insertAt = trailingTemplate.index;
		}
		const trailingNavigation = /(?:^|\n)(\{\{\s*(?:Şablon grubu|[^\n{}]*(?:dolaşım|navbox|kadro))[^\n]*[\s\S]*?\n\}\}\s*)(?=\[\[(?:Kategori|Category):|$)/im.exec( cleanSource );
	...
	function ensureNotesSection( source ) {
		...
		const hasNotes = /\{\{\s*adn\s*\|/i.test( tableText );
		let after = source.slice( table.end );
		after = after.replace( /^(?:[ \t]*\r?\n)*[ \t]*\{\{\s*Not[ _]listesi\s*\}\}[ \t]*(?:\r?\n)?/i, '' );
		const outside = source.slice( 0, table.start ) + after;
		if ( hasNotes && !/\{\{\s*(?:adn|efn)\s*\||<ref\b[^>]*\bgroup\s*=/i.test( outside ) ) {
			after = after.replace( /(?:^|\n)==[ \t]*Notlar[ \t]*==[ \t]*\r?\n\s*\{\{\s*Not[ _]listesi\s*\}\}\s*(?===|$)/i, '\n' );
		}
		if ( hasNotes ) {
			after = after.replace( /^(?:[ \t]*\r?\n)+/, '' );
	...
	function detectInfoboxBounds( source ) {
		...
		const infoboxStart = /\{\{\s*Futbolcu bilgi kutusu\b/i.exec( lines[ startIndex ] );
	...
	function clearInfoboxStatDashes( templateText ) {
		...
		topLevelTemplateParameters( templateText ).reverse().forEach( ( parameter ) => {
			if ( !/^(?:maç|gol|milli?maç|milligol)\d+$/i.test( cleanValue( parameter.name ) ) ||
	...
	function rebuildInfobox( source, rows ) {
		...
		if ( managerParameters.values.length ) {
			...
			for ( let i = 0; i < infoboxLines.length; i += 1 ) {
				if ( /^\|\s*(?:milliyıl|millitakım|milli?maç|milligol)\d+\s*=/i.test( infoboxLines[ i ].trim() ) ) {
					managerInsertAt = i + 1;
				}
			}
		...
		for ( let i = 0; i < infoboxLines.length; i += 1 ) {
			if ( /^\|\s*(?:kulüpyıl|altyapıyıl|milliyıl)\d+\s*=/i.test( infoboxLines[ i ].trim() ) ) {
			...
			normalizeInfoboxParameterSpacing(
				updateInfoboxUpdateParameter(
					reorderInfoboxDisplayParameters(
						removeAutomaticTotalsAndEmptyCareerGroups( infoboxLines.join( '\n' ) )
					)
				)
			)
		);
	...
	function setEditorText( value ) {
		...
		// textSelection, WikiEditor ve CodeMirror tarafından ortaklaşa desteklenen
		// en güvenli içerik yazma katmanıdır.
		if ( window.jQuery && window.jQuery.fn &&
			...
			try {
				...
				mw.log.warn( 'Düzenleyici API’si kullanılamadı:', error );
			}
		}
		if ( !writtenThroughEditorApi && codeMirrorInstance && codeMirrorInstance.view ) {
			try {
				...
				mw.log.warn( 'CodeMirror güncellenemedi:', error );
			}
		}
	...
	function getEditorText() {
		if ( window.jQuery && window.jQuery.fn &&
			...
			try {
				...
				mw.log.warn( 'Düzenleyici içeriği okunamadı:', error );
			}
		}
	...
	async function resolveLoadedTeamRedirects() {
		...
		try {
			...
			mw.log.warn( 'Kulüp yönlendirmeleri çözülemedi; mevcut bağlantılar korundu:', error );
		}
	}
	...
	async function resolveInfoboxRedirects( source ) {
		...
		const parameters = topLevelTemplateParameters( infoboxText ).filter( ( parameter ) => /^(?:altyapıkulübü|çalıştığıkulüp)\d+$/i.test( cleanValue( parameter.name ) )
		);
		const links = [];
		parameters.forEach( ( parameter ) => {
	...
	async function applyToVisualEditor( rows ) {
		...
		try {
			// Form açıldıktan sonra değişen bir belgeye eski istatistikleri uygulama.
			...
			if ( !response.visualeditor || !cleanValue( response.visualeditor.content ) ) {
				throw new Error( 'Güncellenen içerik görsel düzenleyiciye dönüştürülemedi.' );
			}
			...
			if ( model.isReadOnly() || !newDoc.data.hasContent() ) {
				throw new Error( 'Görsel düzenleyiciye değişiklik uygulanamadı.' );
			}
			model.breakpoint();
			try {
				const range = session.doc.getDocumentRange();
				// Tam belge yeniden eklenirken kategori ve diğer üstverileri çoğaltma.
				...
				// Ekleme başarısız olursa aynı işlemdeki silmeyi de geri al.
				model.breakpoint();
			...
			mw.notify(
				'İstatistikler uygulandı. Değişiklikleri görsel düzenleyiciden yayımlayabilirsiniz.'
			);
		} catch ( error ) {
			mw.log.warn( 'FootballerStats görsel düzenleyici hatası:', error );
			mw.notify( error.message || 'Değişiklikler uygulanamadı. Yeniden deneyin.', { type: 'error' } );
		} finally {
	...
	async function applyToTextarea() {
		...
		if ( !cleanValue( editorText ) ) {
			mw.notify( 'Madde metni editörden okunamadı. İçeriğin silinmesini önlemek için işlem durduruldu.', {
				title: TOOL_NAME,
				type: 'error'
			} );
			return;
		...
		try {
			...
			mw.log.warn( 'FootballerStats önizleme hatası:', error );
			mw.notify( error.message || 'Önizleme oluşturulamadı. Yeniden deneyin.', {
				type: 'error'
			} );
		}
	}
	...
	function findPreviewStatsTable( previewRoot ) {
		...
		return Array.from( previewRoot.querySelectorAll( 'table.wikitable' ) ).find( ( table ) => {
			const headings = Array.from( table.querySelectorAll( 'th' ) )
				.map( ( heading ) => cleanValue( heading.textContent ).toLocaleLowerCase( 'tr-TR' ) );
			return [ 'sezon', 'maç', 'gol' ].every(
				( label ) => headings.some(
					( heading ) => heading === label || heading.includes( label )
				)
			) && headings.some( ( heading ) => /kulüp|takım/.test( heading ) );
		} ) || null;
	}
	...
	function enhancePreviewStatsTable( contentRoot = document.querySelector( '#wikiPreview, .mw-preview' ) ) {
		...
		if ( !actions ) {
			...
			button.addEventListener( 'click', ( event ) => {
				...
				if ( ensureUi() ) {
					visualEditorSession = null;
					backdrop.querySelector( '.tfsh-apply' ).textContent = 'Önizle';
					refreshUpdateDate();
					backdrop.classList.add( 'is-open' );
					window.requestAnimationFrame( updateOptionalCompetitionColumns );
				}
	...
	function ensureUi() {
		...
		backdrop.innerHTML = `
      <div class="tfsh-modal" role="dialog" aria-modal="true" aria-label="${ TOOL_NAME }">
        ...
        <table class="tfsh-table">
          <colgroup>
            <col style="width:11%"><col style="width:11%"><col style="width:max(5%, calc(7ch + 22px))"><col class="tfsh-infobox-year-column" style="width:6%"><col style="width:10%"><col style="width:calc(var(--tfsh-league-width) * 8 / 13)">
            <col style="width:calc(var(--tfsh-league-width) * 2.5 / 13)"><col style="width:calc(var(--tfsh-league-width) * 2.5 / 13)">
            <col class="tfsh-local-league-column" style="width:calc(var(--tfsh-local-league-width) * 8 / 13)"><col class="tfsh-local-league-column" style="width:calc(var(--tfsh-local-league-width) * 2.5 / 13)"><col class="tfsh-local-league-column" style="width:calc(var(--tfsh-local-league-width) * 2.5 / 13)">
            <col class="tfsh-national-cup-column" style="width:calc(var(--tfsh-national-cup-width) * 3.7 / 7.4)"><col class="tfsh-national-cup-column" style="width:calc(var(--tfsh-national-cup-width) * 3.7 / 7.4)">
            <col class="tfsh-league-cup-column" style="width:calc(var(--tfsh-league-cup-width) * 3.5 / 7)"><col class="tfsh-league-cup-column" style="width:calc(var(--tfsh-league-cup-width) * 3.5 / 7)">
            <col class="tfsh-continental-column" style="width:calc(var(--tfsh-continental-width) * 3 / 6)"><col class="tfsh-continental-column" style="width:calc(var(--tfsh-continental-width) * 3 / 6)"><col class="tfsh-other-column" style="width:calc(var(--tfsh-other-width) * 2.5 / 5)"><col class="tfsh-other-column" style="width:calc(var(--tfsh-other-width) * 2.5 / 5)"><col style="width:5%">
          </colgroup>
          <thead>
            <tr>
              ...
              <th rowspan="2" class="tfsh-infobox-year-heading tfsh-infobox-year-column">Bilgi<br>kutusu<br>yılı</th>
            ...
            <tr>
              <th>Lig</th>
              <th class="tfsh-stat-heading"><span>Maç</span></th>
              <th class="tfsh-stat-heading"><span>Gol</span></th>
              ...
              <th class="tfsh-other-column tfsh-stat-heading"><span>Maç</span></th>
              <th class="tfsh-other-column tfsh-stat-heading"><span>Gol</span></th>
            </tr>
          </thead>
          <tbody></tbody>
        </table>
        ...
        <div class="tfsh-actions">
          <div class="tfsh-update-date-field">
            <label for="tfsh-update-date-input">Güncelleme tarihi</label>
            <label class="tfsh-link-toggle tfsh-today-toggle">Bugün<input type="checkbox" class="tfsh-update-date-today"></label>
            <input id="tfsh-update-date-input" type="text" class="tfsh-update-date" placeholder="Kariyeri sonlanmıştır" disabled>
          </div>
        ...
        <div class="tfsh-note-dialog-content">
          ...
          <table class="tfsh-note-table">
            ...
            <thead><tr>
              ...
              <th scope="col" class="tfsh-stat-heading"><span>Maç</span></th>
              <th scope="col" class="tfsh-stat-heading"><span>Gol</span></th>
              <th class="tfsh-remove-cell"></th>
            </tr></thead>
            <tbody class="tfsh-note-entries"></tbody>
          </table>
          <button type="button" class="tfsh-inline-link tfsh-note-add" aria-label="Müsabaka ekle" title="Müsabaka ekle">+</button>
    ...
    `;
		...
		const noteDialog = jQuery( noteBackdrop.querySelector( '.tfsh-note-dialog-content' ) ).dialog( {
			...
			closeText: 'Kapat',
			position: { my: 'top', at: 'top+50', of: window },
			buttons: [ { text: 'Kaydet', click: saveCompetitionNote } ],
			close: () => {
		...
		closeAction.$button.addClass( 'tfsh-close' ).attr( 'title', 'Kapat' );
	...
	async function syncUiAvailability() {
		...
		const search = new URLSearchParams( window.location.search );
		// Kullanıcı aracı bağlantıdan açıkça çağırdıysa textarea'nın veya
		// WikiEditor/CodeMirror katmanının geç dolmasını beklemeden modalı aç.
	...
	mw.loader.using( [ 'mediawiki.util', 'mediawiki.api', 'mediawiki.storage', 'jquery.textSelection', 'jquery.ui', 'oojs-ui-core', 'oojs-ui-windows', 'oojs-ui.styles.icons-interactions' ] )
		.then( startTool )
		.catch( ( error ) => {
			mw.log.warn( `${ TOOL_NAME } bağımlılıkları yüklenemedi:`, error );
			startTool();
		} );
}() );
// </nowiki>
